...
from pyrts.core.encodings import *
...
class CacheHandler(object):
    ...
    def set_object(self, key, json, **kwargs):
        return self.cache.set_json(key, json, **kwargs)
...
class BaseCache(object):
    ...
    def validate_key(self, key):
        ...
        if len(key) > MEMCACHE_MAX_KEY_LENGTH:
            print "Cache key will cause errors if used with memcached:  %s (longer than) %s " % (key, MEMCACHE_MAX_KEY_LENGTH)  
        for char in key:
            if ord(char) < 33 or ord(char) == 127:
                print "Cache key contains characters that will cause errors if used with memcached: %s " % (key)  
